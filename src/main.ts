...
let FSM = StateMachine.factory(machine)
let fsm = new FSM()
...
function adAginFn() {
    console.log("再次观看广告")
    pickup(id("again_medium_icon_dialog_ensure_text"), (o) => {
        if (o !== null) {
            press(o.centerX(), o.centerY(), 10)
            setTimeout(function () {
                fsm.playAd()
...
function liveFollowFn() {
    ...
    pickup(className("android.widget.TextView").text("退出直播间"), (o) => {
...
function liveAginFn() {
    pickup(id("again_dialog_ensure_text"), (o) => {
        if (o !== null) {
            press(o.centerX(), o.centerY(), 10)
            setTimeout(function () {
                fsm.playLive();
...
function selectTask(w, name) {
    if (w.id()) {
        const a = pickup(w, className("android.widget.TextView"));
